...
function solve(levelIds: string[], stars = 3): Map<string, LevelProgress> {
  return toProgressMap(
    levelIds.map((levelId) => ({ profileId: 'p', levelId, bestStars: stars, solvedAt: 1, attempts: 1, hintsUsed: 0, lastPlayedAt: 1 })),
  );
}

describe('mergeProgress', () => {
  it('starts a summary and keeps the best result', () => {
    const first = mergeProgress(undefined, { profileId: 'p', levelId: 'l', stars: 0, hintsUsed: 1, at: 10 });
    expect(first).toMatchObject({ bestStars: 0, solvedAt: null, attempts: 1, hintsUsed: 1 });
    const second = mergeProgress(first, { profileId: 'p', levelId: 'l', stars: 2, hintsUsed: 0, at: 20 });
    expect(second).toMatchObject({ bestStars: 2, solvedAt: 20, attempts: 2, hintsUsed: 1, lastPlayedAt: 20 });
    const third = mergeProgress(second, { profileId: 'p', levelId: 'l', stars: 1, hintsUsed: 3, at: 15 });
    expect(third).toMatchObject({ bestStars: 2, solvedAt: 20, attempts: 3, hintsUsed: 3, lastPlayedAt: 20 });
  });
});

describe('unlock rules', () => {
  it('counts stars and completion', () => {
    const progress = solve(meadow.levels.slice(0, 5).map((level) => level.id), 2);
  ...
  it('opens worlds by stars or by a parent', () => {
    expect(worldLock(meadow, SEED_WORLDS, new Map(), profile)).toEqual({ unlocked: true, starsNeeded: 0, afterWorld: null });
    expect(worldLock(forest, SEED_WORLDS, new Map(), profile)).toMatchObject({ unlocked: false, starsNeeded: 15 });
    ...
    expect(worldLock(forest, SEED_WORLDS, new Map(), { ...profile, unlockedWorlds: ['forest'] }).unlocked).toBe(true);
    const gated = { ...forest, unlock: { afterWorld: 'meadow' } };
    expect(worldLock(gated, SEED_WORLDS, some, profile)).toMatchObject({ unlocked: false, afterWorld: 'meadow' });
    const tier3 = { ...forest, tier: 3 as const };
    expect(worldLock(tier3, SEED_WORLDS, some, profile)).toEqual({ unlocked: false, starsNeeded: 0, afterWorld: null });
  });
  ...
  it('opens levels one after another', () => {
    ...
    expect(isLevelUnlocked(meadow, 9, new Map(), { ...profile, unlockedWorlds: ['meadow'] })).toBe(true);
  ...
  it('opens tiers by share of solved levels', () => {
    ...
    const most = solve(SEED_WORLDS.flatMap((world) => world.levels.slice(0, 8).map((level) => level.id)));
