...
describe('reactionLine', () => {
  it('praises in the hero voice', () => {
    expect(reactionLine({ ...base, success: true, stars: 3 }, 'kitten', 0)).toBe(
      'Мур! Получилось!',
    );
  ...
  it('explains failures kindly', () => {
    expect(
      reactionLine(
        { ...base, failure: { kind: 'bump', reason: 'rock', actorId: 'hero', blockId: null } },
        'kitten',
        0,
      ),
    ).toBe('Ой, камень!');
    const goal = {
      goal: { kind: 'collectAll' as const },
      met: false,
      detail: 'starsLeft' as const,
      starsLeft: 2,
    };
    expect(
      reactionLine({ ...base, failure: { kind: 'goal', statuses: [goal] } }, 'kitten', 1),
    ).toBe('Звёздочки ещё остались!');
    expect(reactionLine({ ...base, failure: { kind: 'goal', statuses: [] } }, 'kitten', 0)).toMatch(
      /попробуем/i,
    );
    ...
    expect(
      reactionLine(
        { ...base, failure: { kind: 'error', code: 'x', message: 'Сообщение', blockId: null } },
        'kitten',
        0,
      ),
    ).toBe('Сообщение');
  ...
  it('encodes failures for analytics', () => {
    ...
    expect(
      failureCode({
        ...base,
        failure: { kind: 'bump', reason: 'edge', actorId: 'hero', blockId: null },
      }),
    ).toBe('bump:edge');
    ...
    expect(
      failureCode({
        ...base,
        failure: { kind: 'error', code: 'too_many_steps', message: '', blockId: null },
      }),
    ).toBe('error:too_many_steps');
