...
export function TaskBar({ number, text, hintsLeft, hintsReady, onBack, onHint, onReset, className }: TaskBarProps) {
  return (
    <header className={cn('flex flex-wrap items-center gap-2 sm:flex-nowrap sm:gap-3', className)}>
      ...
      >
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-brand text-xl font-black text-white">{number}</span>
        <span className="line-clamp-2 min-w-0 flex-1 text-lg leading-tight font-extrabold sm:text-xl">{text}</span>
