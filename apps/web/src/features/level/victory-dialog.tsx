...
export function improvementTip(stars: number, blocks: number, rule: StarsRule): string | null {
  ...
  if (target && blocks > target) return `Попробуй решить короче: хватит ${target} ${target === 1 ? 'блока' : 'блоков'}!`;
  if (rule.three?.noBumps) return 'Попробуй пройти, ни во что не врезаясь!';
  return null;
}

export function VictoryDialog({ stars, blocks, rule, heroId, hasNext, worldDone, onNext, onRetry, onMap }: VictoryDialogProps) {
  const tip = improvementTip(stars, blocks, rule);
  useEffect(() => {
    const timers = [0, 1, 2].slice(0, stars).map((index) => setTimeout(() => sfx.star(index), 250 + index * 220));
    const say = setTimeout(() => {
      const praise = worldDone ? 'Ура! Ты прошёл весь мир!' : stars === 3 ? 'Три звезды! Отлично!' : `Звёзд: ${stars}.`;
  ...
  return (
    <KidDialog open onOpenChange={() => undefined} title={worldDone ? 'Мир пройден!' : 'Получилось!'} description={`Звёзд: ${stars} из 3`} hideClose>
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-center gap-3">
          ...
          <KidButton voiceLabel="Ещё раз" icon={RotateCcw} round size="lg" tone="sun" onClick={onRetry} />
          {hasNext ? <KidButton voiceLabel="Дальше" icon={ArrowRight} caption="Дальше" size="xl" tone="go" onClick={onNext} /> : null}
        </div>
      </div>
    </KidDialog>
  );
